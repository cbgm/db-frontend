define([
	...
	'use strict';	
	jQuery.fn.jqte = function(options){	
		...
		// methods
		jQuery.fn.jqteVal = function(value){
			jQuery(this).closest("."+vars.css).find("."+vars.css+"_editor").html(value);
		}
		...
		return this.each(function(){
			if(!jQuery(this).data("jqte") || jQuery(this).data("jqte")==null || jQuery(this).data("jqte")=="undefined")
				jQuery(this).data("jqte",true);
			else 
				jQuery(this).data("jqte",false);
			
			// is the status false of the editor
			if(!vars.status || !jQuery(this).data("jqte"))
			{
				// if wanting the false status later
				if(jQuery(this).closest("."+vars.css).length>0)
				{
					var editorValue	= jQuery(this).closest("."+vars.css).find("."+vars.css+"_editor").html();
					...
					var thisElementTag = jQuery(this).is("[data-origin]") && jQuery(this).attr("data-origin")!="" ? jQuery(this).attr("data-origin") : "textarea";
					...
					jQuery(this).data("jqte",false).closest("."+vars.css).before(thisClone).remove();
			...
			var thisElementTag = jQuery(this).prop('tagName').toLowerCase();
			
			// tag name of origin
			jQuery(this).attr("data-origin",thisElementTag);
			
			// contents of the element
			var thisElementVal = jQuery(this).is("[value]") || thisElementTag == "textarea" ? jQuery(this).val() : jQuery(this).html();
			...
			jQuery(this).after('<div class="'+vars.css+'"></div>');
			
			// jqte
			var jQTE = jQuery(this).next('.'+vars.css);
			
			// insert toolbar in jqte editor

			//build html
			var html =	'<div class="'+vars.css+"_toolbar"+'" role="toolbar" unselectable></div>' +
						'<div role="dialog" class="image_toolbar">' +
						
						'</div>' +
						'<div role="dialog" class="gallery_toolbar">' +
						
						'</div>' +
						'<div class="'+vars.css+'_linkform" style="display:none" role="dialog"></div>' +
			...
			var imageform		= jQTE.find('.image_toolbar');
			var galleryform		= jQTE.find('.gallery_toolbar');
...
//			<div class="image_button" unselectable style="padding: 3px 12px; background: #AAA; border: #333 1px solid; color: #FFF;	font-size: 12px; font-weight: bold;	cursor: pointer; border-radius: 3px; -webkit-border-radius: 3px; -moz-border-radius: 3px; box-shadow: inset 0 1px #EEE; -webkit-box-shadow: inset 0 1px #EEE; -moz-box-shadow: inset 0 1px #EEE; float:left; margin: 8px;">'+vars.button+'</div>
			//costumization
			imageform.append('<div><select class="images_select" size="2" multiple></select><input class="imagetext" type="text" readonly><br><div style="clear:both;"></div></div>');
			galleryform.append('<div><select class="galleries_select" size="2" multiple></select><input class="gallerytext" type="text" readonly><br><div style="clear:both;"></div></div>');

			
			ImageController.getEntries(function (data) {
			
				var select		= jQTE.find('.images_select');
				var html = "";
				if(data.length > 0) {

					for (var i =0; i < data.length; i++) {
						html += '<option value="' + data[i].imageId + '">' +  data[i].name + '</option>';
					}
			...
			GalleryController.getEntries(function (data) {
				galleries = data;
				var select		= jQTE.find('.galleries_select');
				var html = "";
				if(galleries.length > 0) {

					for (var i =0; i < galleries.length; i++) {
						html += '<option value="' + galleries[i].galleryId + '">' +  galleries[i].name + '</option>';
					}
			...
			//costumization
			var imageselect		= imageform.find(".images_select");
			var galleryselect		= galleryform.find(".galleries_select");
			//costumization
			...
			if(thisElementTag!="textarea")
			{
				...
				jQuery(thisElement[0].attributes).each(function(){
			...
			for(var n = 0; n < buttons.length; n++)
			{
				...
				if(vars[buttons[n].name])
				{
					...
					if(buttons[n].name=="format" && jQuery.isArray(vars.formats))
					{
					...
					else if(buttons[n].name=="fsize" && jQuery.isArray(vars.fsizes))
					{
						toolbar.find("."+vars.css+'_tool_'+buttons[n].cls)
					...
					else if(buttons[n].name=="color" && jQuery.isArray(colors))
					{
						toolbar.find("."+vars.css+'_tool_'+buttons[n].cls)
			...
			//costumization
			toolbar.append('<div style="float: left; margin: 0;	padding: 0;	cursor: pointer;" class="jqte_tool_22 unselectable jqte_hiddenField" role="button"><a class="image_icon unselectable"></a></div>');
			toolbar.find('.jqte_tool_22 a').bind('click', function(){
				jQuery('.image_toolbar').toggle();
			});
			
			toolbar.append('<div style="float: left; margin: 0;	padding: 0;	cursor: pointer;" class="jqte_tool_23 unselectable jqte_hiddenField" role="button"><a class="gallery_icon unselectable"></a></div>');
			...
			function replaceSelection(tTag,tAttr,tVal) {
				...
				if (window.getSelection)
				{
					...
					if (selObj.anchorNode && selObj.getRangeAt)
					{
						...
						jQuery(newElement).attr(tAttr,tVal);
						...
						if(tAttr=="style")
							affectStyleAround(jQuery(newElement),tVal);
						// for other attributes
						else
							affectStyleAround(jQuery(newElement),false);
					}
				}
				// for ie
				else if (document.selection && document.selection.createRange && document.selection.type != "None")
			...
			function selected2link()
			{
				if(!toolbar.data("sourceOpened"))
				{
					...
					{
						if(jQuery(e.target).hasClass(vars.css+"_linktypetext") || jQuery(e.target).hasClass(vars.css+"_linktypearrow"))
							linktypeSwitch(true);
					});
					...
					{
						var thisLinkType = jQuery(this).attr(vars.css+"-linktype");
...
//				editor.val( textBefore + html + textAfter );
			...
			function linkGallery() {
				var select_val = parseInt(jQuery( ".galleries_select option:selected" ).val());
				var html = "";
				var gallery;
				for (var i = 0; i < galleries.length; i++) {

					if (galleries[i].galleryId === select_val) {
						gallery = galleries[i];
						break;
					}
				}
...
//				editor.val( textBefore + html + textAfter );
			...
			function linkRecord(thisSelection)
			{
				...
				else
				{
					...
					editor.find("img").each(function(){
						var emptyPrevLinks = jQuery(this).prev("a");
						var emptyNextLinks = jQuery(this).next("a");
			...
			{	
				...
				if(!toolbar.data("sourceOpened"))
				{
					...
					{
						var styleValue = jQuery(this).attr(vars.css + "-styleval"); // the property of style value to be added
			...
			function selected2format()
			{
				...
				formatbar.find("a").click(function()
				{
					...
					formatLabelView(jQuery(this).text());
					
					var formatValue = jQuery(this).attr(vars.css + "-formatval"); // the type of format value
			...
			{
				var jQueryhtmlContent, jQueryhtmlPattern, jQueryhtmlReplace;

				// first remove to unnecessary gaps
				jQueryhtmlContent = strings.replace(/\n/gim,'').replace(/\r/gim,'').replace(/\t/gim,'').replace(/&nbsp;/gim,' ');
				...
				for(c=0; c<5; c++)
				{
					...
					for(var i = 0; i < jQueryhtmlPattern.length; i++)
					{
						jQueryhtmlContent = jQueryhtmlContent.replace(jQueryhtmlPattern[i], jQueryhtmlReplace[i]);
					}
				}

				// if paragraph is false (<p>), convert <p> to <br>
				if(!vars.p)
					jQueryhtmlContent = jQueryhtmlContent.replace(/\<p(|\s+.*?)>(.*?)\<\/p>/ig, '<br/>jQuery2');
				...
				if(!vars.p && !vars.br)
					jQueryhtmlContent = jQueryhtmlContent.replace(/\<p>(.*?)\<\/p>/ig, '<div>jQuery1</div>');

				return jQueryhtmlContent;
			}
			...
			function detectElement(tags){
				...
				var resultdetect=false, jQuerynode = getSelectedNode(), parentsTag;
				
				if(jQuerynode)
				{
					jQuery.each(tags, function(i, val){
						parentsTag = jQuerynode.prop('tagName').toLowerCase();

						if (parentsTag == val)
							resultdetect = true;
						else
						{
							jQuerynode.parents().each(function(){
			...
			function buttonEmphasize(e)
			{
				...
				{				
					if(vars[buttons[n].name] && buttons[n].emphasis && buttons[n].tag!='')
						detectElement(buttons[n].tag) ? toolbar.find('.'+vars.css+'_tool_'+buttons[n].cls).addClass(emphasize) : jQuery('.'+vars.css+'_tool_'+buttons[n].cls).removeClass(emphasize);
				}
				// showing text format
				if(vars.format && jQuery.isArray(vars.formats))
				{
			...
			toolbutton
				...
				.click(function(e){	
					...
					if(jQuery(this).data('command')=='displaysource' && !toolbar.data("sourceOpened"))
					{
						// hide all the toolbar buttons (except the source button)
						toolbar.find("."+vars.css+"_tool").addClass(vars.css+"_hiddenField");
						jQuery(this).removeClass(vars.css+"_hiddenField");
					...
					else
					{
						...
						if(!toolbar.data("sourceOpened"))
						{
							...
							if(jQuery(this).data('command')=='linkcreator')
							{
							...
							else if(jQuery(this).data('command')=='formats')
							{
								if(jQuery(this).data('command')=='formats' && !jQuery(e.target).hasClass(vars.css+"_format"))
									selected2format();
							...
							else if(jQuery(this).data('command')=='fSize' || jQuery(this).data('command')=='colors')
							{
								if(
								(jQuery(this).data('command')=='fSize' && !jQuery(e.target).hasClass(vars.css+"_fontsize")) || // the font-size button
								(jQuery(this).data('command')=='colors' && !jQuery(e.target).hasClass(vars.css+"_color")) // the color button
								)
								selected2style(jQuery(this).data('command'));
							...
							else
							{
								...
								selectionSet(jQuery(this).data('command'),null);
								...
								jQuery(this).data('emphasis')==true && !jQuery(this).hasClass(emphasize) ? jQuery(this).addClass(emphasize) : jQuery(this).removeClass(emphasize);
				...
				.hover(function(e){
					if(vars.title && jQuery(this).data("title")!="" && ( jQuery(e.target).hasClass(vars.css+"_tool") || jQuery(e.target).hasClass(vars.css+"_tool_icon") ))
					{
						jQuery('.'+vars.css+'_title').remove();

						// create the title bubble
						jQTE.append('<div class="'+vars.css+'_title"><div class="'+vars.css+'_titleArrow"><div class="'+vars.css+'_titleArrowIcon"></div></div><div class="'+vars.css+'_titleText">'+jQuery(this).data("title")+'</div></div>');
						
						var thisTitle = jQuery('.'+vars.css+'_title:first');
						var thisArrow = thisTitle.find('.'+vars.css+'_titleArrowIcon');
						var thisPosition = jQuery(this).position();
						var thisAlignX = thisPosition.left + jQuery(this).outerWidth() - (thisTitle.outerWidth()/2) - (jQuery(this).outerWidth()/2);
						var thisAlignY = (thisPosition.top + jQuery(this).outerHeight() + 5);
			...
			editor
				...
				.bind("keypress keyup keydown drop cut copy paste DOMCharacterDataModified DOMSubtreeModified",function()
				{
					...
					if(jQuery.isFunction(vars.change))
						vars.change();
						
					// the feature of placeholder
					if(vars.placeholder && vars.placeholder!="")
						jQuery(this).text()!="" ? placeHolder.hide() : placeHolder.show();
				})
				...
				.focus(function()
				{
					...
					if(jQuery.isFunction(vars.focus))
						vars.focus();
				...
				.focusout(function()
				{
					...
					if(jQuery.isFunction(vars.blur))
						vars.blur();
					...
					if(jQuery.isArray(vars.formats))
						formatLabelView(vars.formats[0][1]);
				});
